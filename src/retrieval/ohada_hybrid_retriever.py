...
class OhadaHybridRetriever:
   ...
   def determine_search_collections(self, query: str, collection_name: str = None, 
       ...
       """
       # Si une collection est explicitement fournie, l'utiliser
       if collection_name:
           return [collection_name]
       
       # Si une partie est spécifiée, utiliser sa collection
       if partie:
           return [f"partie_{partie}"]
       
       # Déterminer si la requête concerne l'OHADA en général plutôt que le plan comptable
       ohada_general_keywords = [
           "ohada", "traité", "institutions", "présentation", "histoire", 
           "qu'est-ce que l'ohada", "acte uniforme", "ccja", "ersuma",
           "organisation", "harmonisation", "africain", "droit des affaires"
       ]
       
       is_ohada_general = any(keyword.lower() in query.lower() for keyword in ohada_general_keywords)
       
       # Si la requête concerne l'OHADA en général, prioriser la collection presentation_ohada
       if is_ohada_general:
           return ["presentation_ohada", "plan_comptable"]
       
       # Par défaut, inclure les collections standard
       return ["plan_comptable", "chapitres", "presentation_ohada"]
